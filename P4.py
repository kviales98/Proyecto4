...
def modulador(bits, fc, mpp):
    ...
    return senal_Tx, Pm, portadoraS, moduladora  
    
#----------------------------------------------------------
def demodulador(senal_Rx, portadoraS, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadoraS, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadoraS, mpp)
...
# 2. Funciones de tiempo x(t)
X_t = np.empty((4, len(time)))
...
# 5. Las 4 variantes de las funciones
for i in A_x:
    #Combinaciones (A1 = 1, A2 = 1) y ( A1 = -1, A2 = -1)
    x_1 = i * np.cos(2*(np.pi)*fc*time) +  i * np.sin(2*(np.pi)*fc*time)
    #Combinaciones (A1 = -1, A2 = 1) y ( A1 = 1, A2 = -1)
    x_2 = -i * np.cos(2*(np.pi)*fc*time) +  i * np.sin(2*(np.pi)*fc*time) 
    X_t[i,:] = x_1
    X_t[i+1,:] = x_2
    plt.plot(time, x_1, lw=2)
    plt.plot(time, x_2, lw=2)       

# 6. Obtención del promedio de las realizaciones
Prom = [np.mean(X_t[:,i]) for i in range(len(time))]
plt.plot(time, P, lw=8,color='C0',label='Promedio de realizaciones')

# 7. Graficar el resultado teórico del valor esperado
E = np.mean(senal_Tx)*time  # Valor esperado de la señal 
plt.plot(time, E, ':', lw=4,color='r',label='Valor teórico')
...
plt.xlim(0, 20000)
plt.title('Espectro de Potencia - Señal Modulada')
